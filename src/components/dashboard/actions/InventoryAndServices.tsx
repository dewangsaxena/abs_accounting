...
const ProfitMarginsHandler = () => {
  ...
  // Profit Margins
  const [profitMargins, __setProfitMarginsState] = useState<ProfitMarginIndex>(
    {}
  );
  ...
  const updateProfitMargins = async () => {
    ...
    httpService
      ...
      .catch((error: any) => {
        setSuccessful(0);
        showToast(toast, false, error.message || UNKNOWN_SERVER_ERROR_MSG);
      })
      .finally(function () {
        setIsLoading(false);
      });
        ...
        <VStack width="100%" alignItems={"left"}>
          <Box width="100">
            <HStack>
              ...
              <Box>
                {wasSuccessful === 1 ? (
                  <BsPatchCheck fontSize="1.5em" color="#9DE18A"></BsPatchCheck>
                ) : wasSuccessful === 0 ? (
          ...
          <HStack width="100%">
            <Box width="50%">
              <_InputLeftElement
                ...
                onBlur={(event: any) => {
                  let temp: string = event.target.value.trim();
                  if (temp.length > 0)
                    setNewItemIdentifierPrefix(event.target.value.trim());
            ...
            actions={[
              {
                ...
                onClick: () => {
                  if (systemConfigMode === MODE_PARTS)
                    window.open("/adjust_inventory", "_blank");
                },
              },
            ]}
          ></ActionGroup>
