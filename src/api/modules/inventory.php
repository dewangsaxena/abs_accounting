...
class Inventory {
    ...
    private const EXEMPT_ITEMS_ID_COMBINED = [...self::EXEMPT_ITEMS_ID_CAT_1, ...self::EXEMPT_ITEMS_ID_CAT_2];
    ...
    private static function validate_prices(array &$prices): bool|string {
        ...
        foreach ($stores as $store) {
            ...
            $prices[$store]['preferredPrice'] = Utils::round(
                $prices[$store]['preferredPrice'],
                2 /* THE PRECISION SHOULD ALWAYS BE 2 HERE */
            );
    ...
    private static function calculate_selling_price(int $store_id, array $existing_prices, array $profit_margins, float $buying_cost, string $item_identifier): array {
        ...
        // Round off selling Price
        $existing_prices[$store_id]['sellingPrice'] = Utils::round(
            $existing_prices[$store_id]['sellingPrice'], 
            2  /* THE PRECISION SHOULD ALWAYS BE 2 */
        );
    ...
    public static function adjust_inventory(array|null $details, int $store_id, PDO|null &$db = null): array {
        ...
        try {
            ...
            foreach ($keys as $key) {
                ...
                if(in_array($item_id, self::EXEMPT_ITEMS_ID_COMBINED) === false && $quantity_to_be_adjusted > self::MAX_QUANTITY_ALLOWED) throw new Exception('Cannot add more than '. self::MAX_QUANTITY_ALLOWED. ' in a single entry.');
