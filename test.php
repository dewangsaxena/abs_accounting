...
function adjust_income_statement(int $store_id): void {
    $db = get_db_instance();
    try {
        $db -> beginTransaction();

        $params = [':store_id' => $store_id, ':_date' => '2025-06-24'];
        $statement = $db -> prepare(<<<'EOS'
        SELECT 
            1 AS `type`,
        ...
        $statement = $db -> prepare('SELECT id, `statement` FROM income_statement WHERE store_id = :store_id AND `date` = :_date;');
        $statement -> execute($params);
        $records = $statement -> fetchAll(PDO::FETCH_ASSOC);
        $is_id = $records[0]['id'];
        $is = json_decode($records[0]['statement'], true, flags: JSON_NUMERIC_CHECK | JSON_THROW_ON_ERROR);
        $is[4020] = $sub_total;
        $is[1520] = $cogs;

        $is = json_encode($is, JSON_NUMERIC_CHECK | JSON_THROW_ON_ERROR);

        // Update 
        $statement = $db -> prepare('UPDATE income_statement SET `statement` = :_statement WHERE id = :id;');
        $is_successful = $statement -> execute([':_statement' => $is, ':id' => $is_id]);
        if($is_successful !== true || $statement -> rowCount() < 1) throw new Exception('Unable to update IS');
...
adjust_income_statement(StoreDetails::CALGARY);
?>  