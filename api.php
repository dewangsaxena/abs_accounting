...
if(isset($_POST['action'])) {
    ...
    switch($data['action']) {
        ...
        /* Client */ 
        case Client::ADD: $response = Client::process($data); break;
        case Client::UPDATE: $response = Client::process($data); break;
        case Client::FETCH: $response = Client::fetch($data); break;

        /* User Management */
        case UserManagement::ADD : $response = UserManagement::add($data); break;
        case UserManagement::AUTHENTICATE : $response = UserManagement::authenticate($data); break;
        case UserManagement::UPDATE_PASSWORD : $response = UserManagement::change_password($data); break;
        case UserManagement::UPDATE_STATUS : $response = UserManagement::update_access_status($data); break;
        case UserManagement::CHANGE_USER_ACCESS_LEVEL : $response = UserManagement::change_user_access_level($data); break;
        case UserManagement::FETCH: $response = UserManagement::fetch($data); break;
        case UserManagement::LOGOUT: UserManagement::logout(); break;

        /* Inventory */ 
        case Inventory::ADD: $response = Inventory::process_item($data); break;
        case Inventory::UPDATE: $response = Inventory::process_item($data); break;
        case Inventory::FETCH : $response = Inventory::fetch($data ?? null, $_SESSION['store_id']); break;
        case Inventory::FETCH_PROFIT_MARGINS : $response = Inventory::fetch_profit_margins(0); break;
        case Inventory::UPDATE_PROFIT_MARGINS : $response = Inventory::update_profit_margins($data); break;
        case Inventory::FETCH_ITEM_DETAILS_FOR_ADJUST_INVENTORY : $response = Inventory::fetch_item_details_for_adjust_inventory($data['search_term'], $_SESSION['store_id']); break;
        case Inventory::ADJUST_INVENTORY : $response = Inventory::adjust_inventory($data['details'] ?? null, intval($_SESSION['store_id'])); break;
        case Inventory::ITEM_DETAILS_FOR_TRANSACTIONS: $response = Inventory::item_details_for_transactions($data['search_term']); break;

        /* Transactions */ 
        case Shared::CREATE_TXN: $response = handle_transactions($data); break;
        case Shared::UPDATE_TXN: $response = handle_transactions($data); break;
        case 'fetch_transaction_by_id': $response = handle_transactions($data); break;
        case 'fetch_sales_invoices_for_client': $response = handle_transactions([
            'action' => $data['action'],
        ...
        break;
        case 'txn_search': $response = handle_transactions($data); break;
        case 'txn_fetch_outstanding_txn_for_receipt': $response = handle_transactions($data); break;
        case Shared::TRANSFER_INVOICE: $response = handle_transactions($data); break;
        case Shared::CONVERT_QUOTE_TO_INVOICE: $response = handle_transactions($data); break;
        case 'txn_email': 
            $params = [
                'action' => 'txn_email', 
        ...
        case 'delete_txn': $response = handle_transactions($data); break;

        /* Customer Statement */ 
        case 'email_customer_statement': $response = CustomerStatement::email(
            $data['clientId'] ?? null,
            $data['storeId'],
            $data['startDate'],
            $data['endDate'],
        ...
        /* Income Statement */ 
        case 'fetch_income_statement': $response = IncomeStatementActions::fetch_graph_data_points(
            $data['startDate'],
            $data['endDate'],
            $data['selectedStores'],
        ); break;
